...
public interface JobPostingRepository extends PagingAndSortingRepository<JobPosting, Integer> {
    @Query(
            value =
                    "select jp from positions p inner join job_postings jp on p.id = jp.positionId where p.companyId = ?1 and p.active = true")
    Page<JobPosting> findAllByCompanyIdIncludeAll(int id, Pageable pageable);
    @Query(
            value =
                    "select jp from positions p inner join job_postings jp on p.id = jp.positionId where p.companyId = ?1 and p.active = true and jp.active = true")
